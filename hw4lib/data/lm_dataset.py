...
class LMDataset(Dataset):
    ...
    def __init__(
        ...
        self.text_files = sorted(glob.glob(os.path.join(self.text_dir, '*.npy')))
    ...
    def collate_fn(self, batch: List[Tuple[torch.LongTensor, torch.LongTensor]]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        lengths = torch.LongTensor([len(shifted) for shifted in shifted_transcripts])

        # TODO: Pad sequences (use torch.nn.utils.rnn.pad_sequence and pad with pad_token)
        padded_shifted = torch.nn.utils.rnn.pad_sequence(shifted_transcripts, batch_first=True, padding_value=self.pad_token)
        padded_golden  = torch.nn.utils.rnn.pad_sequence(golden_transcripts, batch_first=True, padding_value=self.pad_token)
