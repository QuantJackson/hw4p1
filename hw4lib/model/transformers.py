...
class DecoderOnlyTransformer(nn.Module):
    ...
    def forward(self, padded_targets: torch.Tensor, target_lengths: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, dict]:
        ...
        causal_mask = CausalMask(padded_targets) if target_lengths is not None else None
...
class EncoderDecoderTransformer(nn.Module):
    ...
    def encode(self, padded_sources: torch.Tensor, source_lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, dict]:
        ...
        pad_mask_src = PadMask(padded_sources, source_lengths) if source_lengths is not None else None
        ...
        for i in range(self.num_encoder_layers):
            ...
            x_enc, attention = self.num_encoder_layers[i](x_enc, pad_mask_src)
    ...
    def decode(
        ...
        causal_mask = CausalMask(padded_targets, target_lengths) if target_lengths is not None else None
        ...
        for i in range(self.num_decoder_layers):
            if self.training and self.layer_drop_rate > 0 and random.random() < self.layer_drop_rate:
                continue
            # TODO: Pass through decoder layer
            x_dec, self_attn, cross_attn = self.dec_layers[i](x_dec, encoder_output, pad_mask_src, pad_mask_tgt, causal_mask)
